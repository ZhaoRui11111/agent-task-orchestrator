...
test("source CLI completes the local Phase 1 init, Project, Task, backup, restore, restart, and doctor path", async () => {
  ...
  try {
    ...
    assert.match(humanStatus.stdout, /^OK status initialized=true schemaVersion=6 projectCount=1 taskCount=2 dependencyCount=0 grantCount=20 auditCount=[0-9]+\n$/u);
    ...
    assert.equal(backup.body.result.sourceSchemaVersion, 6);
    ...
    assert.equal(restored.body.result.targetSchemaVersion, 6);
