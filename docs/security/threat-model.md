...
security non-claims for the local-first orchestrator. The repository
...
plus a durable injected Phase 3 library with exact tuple and immutable
ownership-digest binding, exact ProjectPolicy/completion/integration/workspace-
v2 ports, separately authorized policy/gate/completion/integration/cleanup
...
`ato.scheduler/v1` register/inspect/remove lifecycle ownership, durable no-
...
| T7 | Duplicate/missed scheduled triggers, substituted scheduler source/dispatcher target, stale Project binding, or dispatcher worker death duplicates execution, skips reconciliation, or loses candidate outcomes. | The current dispatcher enforces the [scheduler contract](../reference/scheduler-contract.md) reconcile-first sequence and reliability claim/fence/idempotency/fan-out records with exact run-owner takeover. Scheduled delivery is at least once: malformed, denied, stale-config, source/target-mismatched, Project-stale, and clock-invalid deliveries create no run, while every duplicate exact `(schedule_id, config_revision, scheduled_for)` attaches to the one durable canonical run. Scheduler source identity is distinct from the Manual execution adapter. Register/remove persist intent before effect and reconcile ambiguity without blind replay. | No concrete scheduler adapter or availability/deadline guarantee exists; an eventual platform adapter or scheduler outage could delay or miss work outside this library boundary. |
...
current-v1 CLI portion of N8. Port/Fake tests cover exact workspace-v2, policy,
completion, integration, and scheduler shapes plus authorization, freshness, durable
transition/restart/ambiguity/fence/corruption/redaction logic. On one exact
Windows/Git host, local adapters cover applicable N1/N2 path, name, repository-
...
| N11 | Missing/false trusted bootstrap, renewal, upgrade, grant-management, Project, restore, Manual-report, scheduler register/remove, completion, integration, push, or workspace-cleanup confirmation; second bootstrap; skipped vocabulary step; wrong local principal/runtime-root identity, action, scope, schedule/config/registration revision, dispatcher/reservation owner, run/member/generation/lease revision, or workspace-root identity, including finalized replay; not-yet-valid, expired, or revoked grant between prepare, Act, observation, Finalize, or dispatcher/scheduler/workspace/Phase-3 transitions; baseline creation or renewal attempted authority expansion; stale lifecycle/effect handoff; cached prior policy/inspection allow/deny; disabled Project policy; replayed request/decision/confirmation; forged scheduler trigger, gate cancellation, receipt, integration observation, or cleanup attestation; Project/Task/repository text that claims authority | No unauthorized Project/Task/dependency/grant/backup/restore/execution/Manual/scheduler/gate/workspace/integration/cleanup effect, dispatcher/reservation claim, result disclosure, or completion; each transition consumes its exact current immutable authorization binding, while scheduler Finalize consumes the allowed final-point-of-use Act decision under current tuple CAS; each inspection obtains a fresh current evaluation; denial never wedges later authorized recovery, and finalized replay revalidates principal/runtime-root identity; baseline creation and never-upgraded renewal create no newer execution, dispatcher, scheduler, workspace, or Phase-3 grant; forged shapes cannot override authoritative durable lineage; bounded denials are atomic and sanitized; content never changes actor, action, policy, confirmation, capability epoch, grant, owner, schedule/config/registration/run/member/generation/lease revision, fence, receipt, attestation, reservation, or completion state. |
