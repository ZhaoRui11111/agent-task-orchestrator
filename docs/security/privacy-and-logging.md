...
The workspace foundation stores only opaque Project/Task/run/member/execution/
workspace/operation identities, positive revisions/generation/attempt/fence/
inventory counts, closed lifecycle/action/state/outcome/reason codes,
timestamps, one receipt digest, and nullable bounded opaque evidence
references. Its dedicated transition-event rows have one exact fixed field set
and no attributes object. Evidence references must be 1–128-character tokens
using only ASCII letters, digits, `.`, `_`, `:`, and `-`, beginning with a
letter or digit. Raw canonical path, repository/registration/branch/HEAD text,
Task/source content, environment, credential, backend message/payload, SQL, and
...
- Primary Task state, audit history, intents, receipts, grants, workspace
  generations, and workspace events are not log data. They remain with the
