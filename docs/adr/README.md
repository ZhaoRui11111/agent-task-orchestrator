...
This directory records accepted architecture decisions for agent-task-orchestrator. Acceptance means that implementation work must satisfy the decision; it does not by itself prove that a capability is implemented, tested, or supported. The repository currently has the qualified local explicit-Manual Phase 2 product runtime, the closed authorized Codex product subset, and separately injected Phase 3 and scheduler libraries described in [ARCHITECTURE.md](../../ARCHITECTURE.md), but no concrete scheduler adapter, default scheduler operation route, validated Codex platform/account support, supported platform, or release.
