...
reliable Manual execution loop, reconcile-first Manual dispatcher, and a pure/
durable workspace foundation with a test Fake plus an exported, product-unwired
Windows Git backend. Entries marked "planned runtime contract" or a
...
| [Persistence contract](persistence-contract.md) | Current implemented foundation and future schema owner | Current SQLite runtime root, single current baseline, Phase 1 metadata/Domain/ProjectRegistry/authorization/application storage, execution attempts/sequences, Manual-loop evidence/journal/completion, explicit-Manual dispatcher records, and workspace generation/operation/evidence records, ingress, connections, transactions, backup/restore and corruption handling; product facade reads do not create a second writer; future tables require their own approved schema change |
| [Reliability protocol](reliability-protocol.md) | Current Manual-loop/dispatcher/workspace/product-facade and planned later-runtime contract | Implemented claim semantic identity, attempts, leases, fencing, idempotency, intent/observation/receipt/finalization, Manual plus Fake/Windows-workspace recovery/retry/CAS, dispatcher fan-out outcomes, and typed product derivation; external publication remains planned |
| [Authorization contract](authorization-contract.md) | Current local authorization contract | Trusted bootstrap and identity, nineteen base, four claim-foundation, six Manual-loop, one dispatcher, and five workspace actions; explicit one-step non-grantable capability upgrades through versions 1 to 5; grants/epochs, narrowing policy, trusted Manual/cleanup confirmations, lifecycle handoff, and fail-closed application/dispatcher/workspace/product decisions; no authority for unimplemented external effects |
| [Adapter contracts](adapter-contracts.md) | Current Execution/Manual/workspace and planned other-port contract | Implemented pure `ato.execution/v1` and `ato.workspace/v1`, production local Manual adapter/control boundary, test-only workspace Fake, exported product-unwired Windows Git workspace backend, current port versions, operation envelopes, receipts, and adapter error taxonomy; scheduler, policy, completion, and product-wired workspace adapters remain planned |
...
| [Completion and workspace contract](completion-workspace-contract.md) | Current durable/physical-workspace and planned completion contract | Implemented isolated generation identity/status/recovery plus product-unwired Windows worktree creation, ownership manifest, authoritative inspection/recovery, path safety, and unconditional cleanup denial; gate freshness, integration reservation, Git publication partial success, and cleanup eligibility remain planned |
| [Observability contract](observability-contract.md) | Current bounded durable/display subset and planned runtime contract | Current durable records, dedicated redacted workspace transition events, and closed current-v1 summaries; planned general operational event sink, diagnostic access, and broader redaction pipeline |
| [Versioning and compatibility contract](versioning-compatibility-contract.md) | Current schema/API/port-version owner and planned product compatibility contract | Current schema-version-1 identity, sole current `ato.api/v1`, independent closed `ato.execution/v1` and `ato.workspace/v1`, the authorized unreleased v1 reset, future evolution, restore boundaries, and evidence-bound support claims |
...
| [Threat model](../security/threat-model.md) | Current local security subset and planned later contract | Assets, actors, trust boundaries, abuse cases, implemented registry/authorization/application/Manual-loop/dispatcher/Fake-and-Windows-workspace mitigations, residual risks, negative-test obligations, and later-runtime non-claims |
| [Privacy and logging contract](../security/privacy-and-logging.md) | Current durable-record/audit/display subset and planned later contract | Data classes, implemented sanitized Phase 1, Manual-loop, dispatcher, workspace evidence, and current product projections, prompt and secret handling, planned operational logging/retention/diagnostic disclosure, and default no telemetry |
