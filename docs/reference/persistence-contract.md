...
model, the authorized Codex product subset, and the fresh-only injected Phase 3
and scheduler libraries in one schema-version-1
...
Codex profiles, product operations, effect authorizations, execution intents,
and package-private turn/terminal journal records,
...
| --- | --- |
| `1`, `0001-current-baseline.sql` | The complete implemented local explicit-Manual Phase 2, authorized Codex product/journal, and fresh-only injected Phase 3 plus scheduler storage model described in this contract |
...
vocabulary version 1 to 2, 2 to 3, 3 to 4, 4 to 5, 5 to 6, 6 to 7, and 7 to 8.
...
- `authorization_capability_epochs` stores one immutable contiguous
  vocabulary-version-1-through-8 renewal/upgrade lineage bound to the exact action-set
  digest.
...
- `application_lifecycle_authorizations` stores immutable short-lived
  ...
  `state_digest_version = 4` of the complete current non-lifecycle application state.
...
path that appends vocabulary-version-2, -3, -4, -5, -6, -7, or -8 origin grants.

Reliable execution-loop storage uses the same epoch/grant relations and the sole
current digest-version-4 `applicationStateProjection`. A confirmed
...
their tuple, backend discriminant, adapter, Project/Task/execution/attempt/fence,
predecessor, complete workspace identity, and operation-specific identities are
immutable. Their closed failure projection
...
`manual_backend_turns` is the retained narrowly mutable Manual adapter state and
...
`execution_operation_intents` discriminates `manual-local`/`none` from
`codex-sdk`/`owned`. Codex intents bind the exact current workspace contract,
ID, generation, revision, trusted root key, ownership digest, HEAD object, and
`task-sha256` input reference. They do not store Task input bytes or a raw
working-directory path. `codex_backend_turns` stores one immutable semantic
turn owner per execution with nullable SDK thread identity, exact predecessor
thread/execution lineage, lifecycle `unknown|active|turn_succeeded|failed`,
nullable terminal signal `turn.completed|turn.failed`, cancellation-request
time, bounded code/evidence reference, revision, and timestamps. Its legal
updates only attach one stable thread, record one cancellation request, mark an
unproved active turn unknown, or record one terminal transition while retaining
the tuple and fence. `codex_backend_operations` is the immutable terminal
operation/receipt lineage; it binds the originating intent/authorization,
idempotency key, exact thread and predecessor, fence, terminal signal,
post-revision, receipt ID, and receipt digest. A cancellation request is not a
terminal operation or proof of `cancelled`.
An execution ID may occur in exactly one backend-turn family: simultaneous
`manual_backend_turns` and `codex_backend_turns` ownership is corrupt even
though the tables have separate SQL uniqueness domains.

Codex product storage adds exactly four current relations:

- `codex_profiles` is the narrowly revisioned Project-scoped configuration
  projection. It binds status, fixed destination and opaque credential
  reference, Project/resource/config/root identities, exact Git/workspace-root/
  private-home path keys and filesystem identities, constructor/configuration
  digests, creator operation, actor, and timestamps. It stores no credential
  value.
- `codex_profile_operations` is immutable activation/deactivation evidence. It
  binds idempotency, expected/resulting revision and status, request/decision/
  audit/confirmation, exact configuration digest, and trusted time. Exact
  replay returns this original result projection rather than the later mutable
  profile state.
- `codex_product_operations` owns the C19 public idempotency key, complete
  canonical original start/resume/retry command JSON and digest, preallocated
  run/member/execution/workspace/intent identities, source continuation tuple,
  profile/configuration/constructor identities, stage, lifecycle, result code,
  canonical bounded terminal result JSON, revision, and timestamps. Immutable
  command and allocation fields cannot be replaced while stage/lifecycle may
  only advance through their closed forward CAS.
- `codex_effect_authorizations` is immutable Prepare/Act evidence bound to the
  exact product operation, profile revision/configuration, planned or current
  run/member/execution/workspace/intent tuple, decision/grant and required-
  grant-set digest, result/reason/policy, confirmation, and trusted time. An
  allowed Act has exactly one pending-intent consumer and a non-null fresh
  confirmation. A confirmation-required denial alone may store a null
  confirmation; every allow and every other denial requires one.

Dispatcher runs and members discriminate `manual-explicit`, `manual-scheduled`,
`codex-start`, and `codex-continuation` route ownership. Existing Manual/
scheduled claimed members remain `execution-start-intent`; Codex claimed
members use `codex-product-operation` and bind their preallocated product
operation/execution before workspace creation. The closed owner union is
checked by SQL guards and combined decoding rather than inferred from IDs.

All execution evidence is bounded IDs, enums,
...
execution operation, Manual or Codex turn, receipt, finalization, terminal, or
completion row. Fresh schema creation does not bootstrap application state. A separately
...
A separately confirmed version-5-to-6 upgrade or later version-6 renewal
appends one epoch and exactly forty-seven current origin grants. Baseline
...
A separately confirmed version-7-to-8 upgrade or later version-8 renewal
appends one epoch and exactly fifty-five current origin grants. Baseline
creation, bootstrap, and versions 1 through 7 renewal add no `codex.*`
authority.
...
- `workspace_events` stores only the closed prepared/denied/executing/observed/
  ...
  reference; and
- `workspace_cleanup_attestations` stores the exact application-issued
  `ato.workspace-cleanup-attestation/v1` tuple/digests through its unique
  identity-matching cleanup-intent FK.
...
Git backend, but neither is a persistence writer: the schema stores only its
closed digest/receipt projection, never a raw physical manifest, canonical path,
or Git payload.

Phase 3 adds these current record families through the same combined owner:

- `project_policy_receipts` stores the exact query/action, actor, Project and
  subject revisions, policy/config/adapter identity, allow/deny/defer result,
  bounded required facts, receipt digest, validity and evidence reference;
- `completion_gate_requests`, `completion_gate_authorization_decisions`,
  `completion_gate_intents`, `completion_gate_observations`,
  `completion_gate_verified_receipts`, `completion_gate_finalizations`, and
  `completion_gate_events` store the exact gate prepare/Act/observe/verify/
  Finalize lineage and closed evidence projection;
- `completion_decisions` is the immutable generic `manual|policy_gated` parent;
  `manual_completion_decisions` and `policy_gated_completion_decisions` are its
  mutually exclusive exact children. Tasks and `execution_terminal_states`
  reference only the generic parent;
- `integration_target_sequences`, `integration_reservations`,
  `integration_operation_requests`, `integration_authorization_decisions`,
  `integration_intents`, `integration_observations`,
  `integration_verified_receipts`, `integration_finalizations`, and
  `integration_events` store per-target fencing, lease/reservation state, exact
  effect/recovery lineage, closed source-first observations, and terminal
  outcomes; and
- the cleanup attestation relation above stores only after the unique cleanup
  intent exists and the final authorization transaction atomically advances it
  to `executing`.

All Phase 3 rows are `STRICT`, bounded, and covered once by the combined reader
and digest-version-4 projection. Append-only guards, subtype/FK checks, partial
...
| `codex_profiles`, `codex_profile_operations`, `codex_product_operations`, `codex_effect_authorizations` | `src/persistence/application-repository-transaction.ts` only inside the Codex product application after complete parsing, trusted identity/configuration inspection, authorization, and exact stage selection | `application-repository-readers.ts`, `application-repository-state.ts`, digest-version-4 `application-repository-digest.ts`, the Codex product/reliable owners, backup verification, and doctor |
...
No CLI surface opens SQLite or writes these paths directly. Product owners use
only the typed transaction/repository boundary. The Manual and package-private
Codex adapters reach only their declared journal writers and
...
Manual journal, Codex profile/product/effect and turn/terminal-operation journal, terminal fact,
...
authorization, Manual turn/operation lineage, exact Codex profile/C19 command/
product-stage/Act-consumer and intent/workspace/thread/predecessor/terminal-
signal/operation lineage, unique confirmation consumption,
...
twenty-three, twenty-nine, thirty, thirty-five, forty-seven, fifty, or fifty-five direct grants for
vocabulary versions 1, 2, 3, 4, 5, 6, 7, or 8 respectively. Missing, substituted, duplicate-action, wrong-epoch, or
...
digest-version-4 application projection; backup, restore verification, and
lifecycle authorization therefore bind it without a second projection.

The same projection includes each Codex profile, profile operation, product
operation, effect authorization, intent, turn, and terminal operation exactly
once. The decoder rejects a Codex row with a Manual/no-workspace intent,
missing ready workspace lineage, changed input/ownership/HEAD/fence tuple,
replaced thread, invalid predecessor, cancellation-as-terminal assertion,
terminal signal/lifecycle mismatch, cross-family execution ownership, or a
substituted terminal receipt identity/digest. For every terminal operation it
reconstructs the canonical bounded receipt projection from the originating Act
request, intent, turn, and stored receipt ID, then requires the recomputed SHA-256
to equal the stored digest. It independently reconstructs each complete C19
command tuple and digest, profile configuration binding, effect Prepare/Act
tuple, nullable-confirmation rule, single-consumer intent binding, closed stage/
lifecycle/result combination, and canonical stored terminal result. Backup,
...
verified/finalization/event lineage; generic completion parent and exactly one
matching Manual or policy-gated child; Task/terminal-execution completion
convergence; one current integration reservation per target, monotonic fencing,
legal intent/status transitions, source-first receipt code/object/nullability
semantics, and terminal old-intent-before-reservation recovery; and one exact
cleanup attestation bound to its unique permitted-phase cleanup intent. A
terminal reservation with a nonterminal intent, cross-subtype completion,
polymorphic authorization mismatch, duplicate gate, impossible observation,
or attestation/quiescence drift is `CORRUPT_ROW`.
...
a possible Manual or package-private Codex journal call. Executing CAS-binds a fresh Act decision;
...
Each Phase 3 policy evaluation stores its preliminary authorization and exact
receipt without an effect. Gate and integration prepares persist the applicable
request/decision/intent/event before adapter access; Act, observation,
verification and Finalize are separate bounded transactions around an external
call. Reservation acquire/renew/takeover/release and ambiguity recovery use
single exact owner/revision/fence CAS transitions. Policy-gated completion
atomically inserts the generic and policy-gated decisions, Domain completion,
terminal execution fact and audit. Cleanup final authorization atomically
inserts its attestation and advances the unique pending intent to executing;
successful finalization consumes only an independently verified digest echo.
Persistence enforces these facts but never evaluates ProjectPolicy, freshness,
Git state, path ownership, or authorization itself.
...
| --- | --- | --- |
| `1` | LF | `6F336B76BFA2A526A69D66B8A8FF554FE45E3415F4852A259EB4345455935E67` |

The pre-EP-03F development baseline checksum
`D7CDF784C090773CC846D484A8208A0D78FB8F640D8BA37A5827122AB70A4C3A`,
pre-EP-03E checksum
...
are named noncurrent identities for refusal evidence only; neither has a
migration, reader, adoption, or repair path.
...
Codex profile/product/effect plus package-private intent/thread/terminal
records, dispatcher records, and
...
typed Manual, Codex, and injected Phase 3 facades compose their records without
adding a second writer. This does not establish a release, Windows or Codex
support, administrator-managed effective-configuration attestation,
product-wired scheduler operation route, concrete Scheduler adapter,
